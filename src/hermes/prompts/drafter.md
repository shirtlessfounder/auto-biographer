...
- Use line breaks in `draft_text` when it sharpens the read — separate a setup from a punchline, or two contrasting beats, with a single `\n` (or `\n\n` for a paragraph gap). A tweet with 2-3 distinct thoughts should almost always use line breaks instead of running as one block. Each newline costs 1 character against the 280 limit.
