...
function mapCommand(commandText: string): {
  ...
  const editMatch = commandText.match(/^edit to\s+(.+)$/is);

  if (!editMatch) {
    return null;
  }

  const payload = editMatch[1].trim();
...
export function formatCandidatePackageMessage(input: CandidatePackageMessageInput): string {
  const lines = [
    ...
    input.deadlineAt ? `Deadline: ${typeof input.deadlineAt.toISOString === 'function' ? input.deadlineAt.toISOString() : input.deadlineAt}` : null,
    input.publishAt ? `Auto-posts at: ${typeof input.publishAt.toISOString === 'function' ? input.publishAt.toISOString() : input.publishAt} (unless you skip/hold/edit)` : null,
    ...
    'Reply with: skip | post now | edit to ... | another angle',
  ];
